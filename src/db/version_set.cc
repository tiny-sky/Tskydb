...
// file1：smallest = (k1, seq1, type1), largest = (k5, seq2, type2)
// file2：smallest = (k5, seq3, type1), largest = (k8, seq4, type2)
//
// largest_key = (k5, seq2, type2)
//
// file1 -> k1 != k5
// file2 -> k5 = k5 && seq3 > seq2
...
Compaction *VersionSet::PickCompaction() {
  ...
  if (level == 0) {
    // Build the input[0] layer of compaction
...
void VersionSet::SetupOtherInputs(Compaction *c) {
  ...
  // optimization : Add nearby Compaction files to input[0]
  AddBoundaryInputs(icmp_, current_->files_[level], &c->inputs_[0]);
  GetRange(c->inputs_[0], &smallest, &largest);

  // Build the input[1] layer of compaction
  ...
  if (!c->inputs_[1].empty()) {
    ...
    const int64_t input0_size = TotalFileSize(c->inputs_[0]);
    const int64_t input1_size = TotalFileSize(c->inputs_[1]);
    const int64_t expanded0_size = TotalFileSize(expanded0);
    if (expanded0_size > input0_size &&
        expanded0_size + input1_size <
